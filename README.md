...
The CAD runtime lives in the plugin rather than Theme Component uploads. Discourse can prune or replace upload-backed asset URLs, while plugin `public/` assets keep a stable path across browsers and caches. Rebuild this plugin before installing a CAD Viewer component that references a newer runtime version. Version `1.5.2` publishes Viewer runtime `v046` with DXF local preview support, embedded-preview fit fixes, clean model-only previews, and persistent CAD model endpoints for post embeds.
